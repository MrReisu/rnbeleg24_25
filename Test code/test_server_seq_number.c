...
// Funktion zur Überprüfung der Sequenznummer
void checkSequenceNumber(const char *message, int sock, struct sockaddr_in6 *src_addr, socklen_t src_addr_len, int *expected_seq) {
    int seq_num = atoi(message);  // Sequenznummer extrahieren
    char nack_message[BUF_SIZE];
    ...
    if (seq_num == *expected_seq) {
        ...
        // NACK senden
        snprintf(nack_message, sizeof(nack_message), "NACK %d", *expected_seq);
        if (sendto(sock, nack_message, strlen(nack_message), 0, (struct sockaddr *)src_addr, src_addr_len) < 0) {
            perror("sendto (NACK)");
        } else {
            printf("Sent NACK for sequence %d\n", *expected_seq);
        }
    }
}
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (strcmp(buffer, "HELLO") == 0 || strcmp(buffer, "CLOSE") == 0) {
            ...
            // Überprüfe Sequenznummer
            checkSequenceNumber(buffer, sock, &src_addr, src_addr_len, &expected_seq);
        }
    }
